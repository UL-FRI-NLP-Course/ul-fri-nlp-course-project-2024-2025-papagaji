...
def prompts_and_responses(year=2024, chosen_txts_dir='out/chosen_txts/'):
    ...
    file_list = []
    index = 0
    df = pd.read_excel('data/Podatki - PrometnoPorocilo_2022_2023_2024.xlsx', sheet_name=str(year), engine='openpyxl',
                       skiprows=0)
...
input, output, file_list = prompts_and_responses(2023, '2023/')

data = []

for i, line in enumerate(input):
    data.append({'file': file_list[i].name, 'input': line, 'output': output[i]})
...
save_jsonl(data, 'evaluation.jsonl')
